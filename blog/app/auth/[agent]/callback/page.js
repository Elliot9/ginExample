...
export default function AuthCallback() {
  ...
  const handleAuth = async (code, agent, state) => {
    try {
      ...
      setError(error.message || '登錄過程中發生錯誤');
    }
  };
  ...
  return (
    <div className="bg-white rounded-lg shadow-md w-full max-w-md overflow-hidden">
      <div className="p-8">
        <h2 className="text-2xl font-semibold text-center text-gray-800 mb-6">請稍候</h2>
        {error ? (
      ...
      {params.agent && (
        <div className="bg-gray-100 p-4 flex items-center justify-center">
          {getIcon(params.agent)}
          <span className="ml-2 text-gray-600">使用 {params.agent} 登錄</span>
        </div>
      )}
    </div>
  );
